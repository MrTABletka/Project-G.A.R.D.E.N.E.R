...
level_passed_image = pygame.image.load('level_passed.png')
...
level_died_image = pygame.image.load('level_died.png')
run_sound = pygame.mixer.Sound('music/run_sound.mp3')
run_sound.set_volume(1.5)
gun_sound = pygame.mixer.Sound('music/gun_sound.mp3')
gun_sound.set_volume(0.25)
gun_reload = pygame.mixer.Sound('music/gun_reload.mp3')
hit_sound = pygame.mixer.Sound('music/hit_sound.mp3')
level_died_image = pygame.transform.scale(level_died_image,
                                               (
                                               level_died_image.get_width() * 2,
                                               level_died_image.get_height() * 2))
level_passed_image = pygame.transform.scale(level_passed_image,
                                               (
                                               level_passed_image.get_width() * 2,
                                               level_passed_image.get_height() * 2))
background_final_died = pygame.transform.scale(background_final_died,
                                               (
                                               background_final_died.get_width() * 2,
                                               background_final_died.get_height() * 2))
background_final_success = pygame.transform.scale(background_final_success,
                                               (
                                               background_final_success.get_width() * 2,
                                               background_final_success.get_height() * 2))
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if keystate[pygame.K_LSHIFT]:
            pygame.mixer.Channel(0).play(pygame.mixer.Sound('music/run_sound.mp3'))
            self.multiplier = 2
    ...
    def change_weapon(self, num):
        ...
        self.gun_num = num
        pygame.mixer.Channel(2).play(pygame.mixer.Sound('music/gun_change.mp3'))
    ...
    def shoot(self, x, y):
        ...
        ammo_used += 1
        #pygame.mixer.Channel(1).play(pygame.mixer.Sound('music/gun_sound.mp3'))
        gun_sound.play()
...
def main_game(map):
    ...
    #фоновая музыка с рандомом
    play_list = []
    play_list.append('music/soundtrack1.mp3')
    play_list.append('music/soundtrack2.mp3')
    play_list.append('music/soundtrack3.mp3')
    play_list.append('music/soundtrack4.mp3')
    play_list.append('music/soundtrack5.mp3')
    play_list.append('music/soundtrack6.mp3')
    play_list.append('music/soundtrack7.mp3')

    song_number = 1

    random.shuffle(play_list)
    pygame.mixer.music.load(play_list[song_number])
    ...
    while running:
        ...
        if player.hit_points <= 0:
            level_died_time = pygame.time.get_ticks()
            while pygame.time.get_ticks() - level_died_time < 3000:
                screen.blit(background_final_died, (0, 0))
                screen.blit(level_died_image, (
                WIDTH / 2 - level_died_image.get_width() / 2, HEIGHT / 2 - level_died_image.get_height() / 2))
                pygame.display.flip()
            running = False
                ...
                if event.key == pygame.K_e:
                    for i in fires:
                        if pygame.sprite.collide_rect(i, player):
                            level_passed_time = pygame.time.get_ticks()
                            while pygame.time.get_ticks() - level_passed_time < 3000:
                                screen.blit(background_final_success, (0, 0))
                                screen.blit(level_passed_image, (
                                    WIDTH / 2 - level_passed_image.get_width() / 2,
                                    HEIGHT / 2 - level_passed_image.get_height() / 2))
                                pygame.display.flip()
                            running = False
                            for e in all_sprites:
                                e.kill()
